...
{- GHOUL programs has variables in them. To keep track of what each
...
instance Alternative Eval where
  empty = MkEval (\_ _ -> Nothing)
  MkEval e1 <|> MkEval e2 =
    MkEval (\prg env -> case e1 prg env of
                          Nothing -> e2 prg env
                          Just r  -> Just r)

{- Linking matching and evaluation scopes: -}
...
{- 4.17 Evaluating programs
   ...
   implementation of 'eval' for expressions. -}
